...
typedef volatile struct {
    uint32_t play_ctrl;      // 0x00: Format, Loop, KeyOn/Off, Addr(High)
    uint32_t sample_addr;    // 0x04: Sample Address (Low 16 bits)
    uint32_t loop_start;     // 0x08: Loop Start Offset
    uint32_t loop_end;       // 0x0C: Loop End Offset
    uint32_t amp_env1;       // 0x10: AR, D1R
    uint32_t amp_env2;       // 0x14: D2R, RR, DL
    uint32_t pitch;          // 0x18: OCT, FNS
    uint32_t lfo_ctrl;       // 0x1C: LFO Frequency, Depth, Waveform
    uint32_t dsp_send;       // 0x20: IMXL (Level), ISEL (Index)
    uint32_t pan_vol;        // 0x24: Volume (TL), Pan
    uint32_t vol2_flags;     // 0x28: Additional flags
    uint32_t filter_env;     // 0x2C: Digital Filter Envelope
    uint32_t reserved[20];   // Pad to 0x80 bytes
} aica_slot_t;
```
...
**Sample Address Composition**: The full 23-bit sample address is formed by combining bits from two registers:
```
FullAddress = (play_ctrl[6:0] << 16) | sample_addr[15:0]
```

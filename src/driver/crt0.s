...
fiq_timer:
    # Use r10 as scratch to preserve r8/r9 for later use if needed? No, FIQ mode has own r8-r14.
    # But wait, we aren't saving registers!
    # If main loop uses r8/r9, we corrupt them here!
    
    # SAVE REGISTERS (Critical Fix)
    # stmdb sp!, {r0-r7, lr} 
    
	# Increment global timer variable at last word of AICA RAM (AICA_RAM_END - 4)
	ldr	r8,aica_clock
	ldr	r9,[r8]
	add	r9,r9,#1
	str	r9,[r8]
	...
	# 0x00802880 = Timer Control Block
    # 0x00802890 = Timer A Data
    # 0x008028A4 = Timer A/B/C Control? (KOS says 0x24 offset)

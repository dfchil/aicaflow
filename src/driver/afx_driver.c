...
static inline void driver_init(void) {
    ...
    *AICA_VIRTUAL_CLOCK = 0;
    ...
    /* Initialize Stack Pointer (SP) to the top of our reserved mini_stack.
       ...
       - Use r0 temporary to avoid clobbering other registers.
    */
    __asm__ volatile (
        "add r0, %0, #256\n\t"  /* r0 = mini_stack + 256 */
        "mov sp, r0\n\t"       /* SP = r0 (top of stack region) */
        : 
...
static inline void cmd2chnl(volatile afx_flow_state_t *flow, const afx_cmd_t *cmd) {
    uint32_t base_ptr = AICA_REG_BASE + (afx_channel_map_get(flow, (uint32_t)cmd->slot) << 7);
